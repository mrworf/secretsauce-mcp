...
| 2 | completed | this commit | 55/55 OAuth/auth tests, 6/6 browser-session integrations, and 6/6 focused source/admission tests; request-boundary source resolution on gateway/control; direct spoof resistance; configured forwarding source separation; malformed-header pre-auth rejection; shared fixed global window; shared unauthenticated/password concurrency; uniform 429 plus `Retry-After`; canonical source propagated through local login/enrollment/step-up and local MCP OAuth | — |
| 3 | completed | this commit | 125/125 focused source/config/OAuth/authentication/settings/enrollment/break-glass/browser/migration tests; production build; durable rolling 24-hour UUID-keyed evidence; duplicate-correlation concurrency safety; exact/lowered threshold evaluation; final-superadmin suspension; transactional browser/restricted/OAuth invalidation and audit; success/recovery/break-glass/disable clearing; bounded Security workspace field and operator/source-trust documentation | — |
| 4 | completed | this commit | production build; 167-file/1,073-test full regression; current generated control OpenAPI; 14-artifact v2.1 readiness validation; 648-file closed-scope release scan; explicit CPU-bound test budgets for stable whole-suite qualification | The release scan required the sandbox's approved Git permission; the scan itself passed unchanged. |
