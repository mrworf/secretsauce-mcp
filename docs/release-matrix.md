...
| Full unit/integration/browser/security regression | Maintainer | `npm test`; 169 files / 1,097 tests passed | pass |
...
| Production dependency advisory threshold | Security owner | Authorized `npm audit --omit=dev --audit-level=moderate`; zero production advisories after MCP SDK/Hono remediation and removal of React Router | pass |
...
| Artifact/privacy prohibited-data scan | Security owner | 663 tracked, staged, built, generated, and synthetic closed-scope files | pass |
...
| Official Compose clean setup and recreation | Operations owner | A prior candidate completed enrollment/login/session/logout/recreation HTTP contracts, vault isolation, and both actual rotations. Candidate `bfac16c` passes rootless image build/smoke but still requires exact-candidate authenticated MCP and the complete durable/ephemeral matrix in the [runbook](v2.1-release-qualification.md) | pending |
