...
Automated evidence below was collected from executable candidate `7e3a2fd` on
...
| Official Compose clean setup and recreation | Operations owner | Rootless Docker 29.6.1 / Compose 5.3.1 clean build reached bounded enrollment setup; vault network isolation and read-only inventories passed; forced recreation preserved 11 generated files. [Exact-candidate runbook](v2.1-release-qualification.md) still requires interactive enrollment, operational MCP, complete durable/ephemeral state checks, and both rotations | pending |
