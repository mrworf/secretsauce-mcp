...
| 3 | completed | this commit | 9/9 focused identity-envelope/rotation tests; server build; exact closed schemas for confirmed TOTP, pending TOTP, and OIDC flows; shared application-writer-lock exclusion; bounded cross-table cursor batches; complete-row conditional updates; TOTP generation and row-version advancement; OIDC row-version advancement; multi-row transaction rollback/resume; corruption, unexpected-root, schema-drift, and invalid-input rejection; new-root-only decryption | The three current identity-root consumers are explicitly allowlisted; any column drift fails preflight until the maintenance adapter is reviewed. |
| 4 | completed | this commit | 28/28 focused root-state/store/coordinator tests plus 9/9 vault lifecycle tests; both targets; populated vault and all three identity stores; writer-lock exclusion; every journal/key/activation/batch/verification/switch/manifest/remove interruption; argument-free resume and completed replay; credential absence through commit/drop; exact config negatives; 162-file/1044-test full suite; production build; current OpenAPI; 14-artifact readiness; 635-file release scan | Docker is unavailable on this host, so the documented temporary identity-database mount override and clean Compose recreation remain a Milestone 10 container release gate; static Compose contracts and real process/Unix-socket lifecycle tests passed. |
